...
function fill_update_modal(iID){
	$.ajax({
			...
			success: function (data) {
				...
				$('#itemdes-update').val(data.item_des);
				$('#amount-update').val(data.item_amount);
				$('#purDate-update').val(data.item_purdate);
				$('#warDate-update').val(data.item_wardate);
				$('#valueDate-update').val(data.item_value);
...
$(document).on('submit', '#update-item-form', function(event) {
	...
	var form_data = new Array(
								...
								$('input[id=itemdes-update]'), 
								$('input[id=amount-update]'), 
								$('input[id=purDate-update]'),
								$('input[id=warDate-update]'),
								$('input[id=valueDate-update]'),
								$('#empID-update'),
								$('#catID-update'),
								$('#conID-update'),
								$('#iID')
...
$(document).on('submit', '#add-avl-form', function(event) {
	event.preventDefault();
	/* Act on the event */
	var validate = '';
	var form_data = new Array(
	...
	if(validate == '01234'){
		$.ajax({
				url: '../data/add_avl.php',
				type: 'post',
				dataType: 'json',
				data: { data: JSON.stringify(data) },
				success: function (response) {
					if(response.valid == valid){
						$('#modal-add-avl').modal('hide');
						$('#modal-message-box').find('.modal-body').text(response.msg);
						$('#modal-message-box').modal('show');
						$('#add-avl-form').trigger('reset');
						 // window.location="../admin/employee.php";
						show_avl();
...
/// Show availability according to office

$(document).on('change', '#avl-choice', function(event) {
	event.preventDefault();
	/* Act on the event */
	var choice = $('#avl-choice').val();
	$.ajax({
			url: '../data/show_avl.php',
			type: 'post',
			data: {
				choice: choice
			},
