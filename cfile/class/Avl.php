...
require_once('../interface/iAvl.php');


class Avl extends Database implements iAvl {
	...
	public function insert_avl($iN, $off, $pos, $avl, $def)
	{
		...
		$sql = "INSERT INTO tbl_avl
				(item_id, off_id, pos_id, avl, def)
				VALUES(?, ?, ?, ?, ?);
				";
		return $this->insertRow($sql, [$iN, $off, $pos, $avl, $def]);	
	}

// 	public function update_employee($fN, $lN, $pos, $off, $type, $eid)
// 	{
// 		$sql = "UPDATE tbl_employee
// 				SET emp_fname = ?, emp_lname = ?, pos_id = ?, off_id = ?, type_id = ?
// 				WHERE emp_id = ?;
// 		";
// 		return $this->updateRow($sql, [$fN, $lN, $pos, $off, $type, $eid]);
// 	}
